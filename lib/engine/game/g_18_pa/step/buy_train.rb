      ...
      module Step
        class BuyTrain < Engine::Step::BuyTrain
          ...
          def buyable_trains(entity)
            ...
            super.reject { |train| train.reserved && train.owner == @game.depot }
          end

          def spend_minmax(entity, train)
            return [train.price, train.price] if train.from_depot?
            return [1, entity.cash] unless entity.cash.zero? && must_buy_train?(entity) && !@last_share_sold_price
          ...
          def process_buy_train(action)
            ...
            if !action.train.from_depot? && action.price > action.entity.cash
              unless action.entity.cash.zero? && must_buy_train?(action.entity) && action.price <= action.entity.owner.cash
